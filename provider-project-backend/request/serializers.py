...
class RequestSerializers(ModelSerializer):
    ...
    _lot = LotsSerializers(source='lot', read_only=True)
    
    create_date = serializers.DateTimeField(format="%d-%m-%Y")

    class Meta:
        model = Request
        fields = ("id", "company",  "profile", "user_bind_typeit", "create_date",  "billing_date",  "payment_date",
                  "return_date", "payment_methods", "amount", "amount_paid",  "amount_commission",  "commission_customer",
                  "freight",  "discount",  "request_number", "observation", "_profile" , "_status", "_catalog" , "_campaign", "_lot")
