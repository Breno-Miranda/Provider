...
class RequestViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        if _pagination:
            ...
            queryset = RequestSerializers(Request.objects.all().filter(
                Q(company_id=companyId) | Q(id=requestId),
                lot__is_open=True).order_by('-id'),
                                          many=True)
    ...
    def list(self, request):
        ...
        try:
            ...
            if not companyId:
                return Response(
                    {
                        "error": "Obrigatorio a informação da empresa.",
                        "status": False
                    },
                    status=HTTP_404_NOT_FOUND)
            ...
            if not requestId:
                return Response(
                    {
                        "error": "Obrigatorio a informação do pedido.",
                        "status": False
                    },
                    status=HTTP_404_NOT_FOUND)
            ...
            queryset_requets = RequestSerializers(
                Request.objects.get(company=companyId, id=requestId))

            queryset_itens = ItensSerializers(
                Itens.objects.all().filter(request_id=requestId), many=True)
            ...
            return Response(
                {
                    "error":
                    "Aconteceu algum inesperado. Entre em contato com o suporte.",
                    "status": False
                },
                status=HTTP_404_NOT_FOUND)
...
class routineRequestsSumItensViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        try:
            ...
            result = Itens.objects.values('request').annotate(
                amount=Sum('total')).filter(request__company=companyId)

            for item in result:
                Request.objects.filter(pk=item['request']).update(
                    amount=item['amount'])

            return Response(
                {
                    "success":
                    "Os valores dos pedidos foram atualizados com sucesso.",
                    "result": result
                },
                status=HTTP_200_OK)
