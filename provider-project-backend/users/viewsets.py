...
class CollaboratorUsersViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            queryset_colaborator = ColaborationUsersSerializers(Collaborator.objects.get(company_id=companyId , user_id=user_id), context={"request": request})
            
            queryset = dict()
            queryset.update(queryset_colaborator.data)
            ...
            return Response({'msm':"Oops! listagem validada e encontrada um error.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND) 
    
    def create(self, request, pk):
            
        if not request.user.has_perm('users.create_profile'):
            return Response({'msm':'Sem permissão de criação. Você será redirecionad(a) para pagina principal.' , 'status': 'danger' , 'return': True},status=HTTP_403_FORBIDDEN) 

        try:
            serializerCollaboration = ColaborationUsersSerializers(data=request.data)
            ...
            return Response({'msm':"Oops! Houve um erro no cadastro.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND) 
    ...
    def update(self, request, pk=None):
        ...
        try:
            _collaborator = Collaborator.objects.get(id=pk, company_id=request.data["company_id"] , user_id=request.data["user_id"])
            serializerCollaboration = ColaborationUsersSerializers(_collaborator, data=request.data)
            if serializerCollaboration.is_valid():
                serializerCollaboration.save()
                
                if request.data['photo']:
                    _collaborator.photo = request.data['photo']
                    _collaborator.save()
                if request.data['anexo']:
                    _collaborator.anexo = request.data['anexo']
                    _collaborator.save()
                
                _contact = Contact.objects.get(id=request.data["contact_id"], company_id=request.data["company_id"] , user_id=request.data["user_id"] , main=1 , is_active=1)
                serializerContact = ContactsSerializers(_contact, data=request.data)
                if serializerContact.is_valid():
                    serializerContact.save()
        
                _bankaccount = Bank_Account.objects.get(id=request.data["bankaccount_id"], company_id=request.data["company_id"] , user_id=request.data["user_id"] , main=1 , is_active=1)
                serializerBankAccount = BankAccountUsersSerializers( _bankaccount ,data=request.data)
                if serializerBankAccount.is_valid():
                    serializerBankAccount.save()
                
                return Response({'success': 'Heey! Ação efetuado acom sucesso.', 'status': True},status=HTTP_200_OK)
        except:
            return Response({'msm':"Oops! Houve um erro no atualização.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND) 
            
    def destroy(self, request, pk=None):
        
        if not request.user.has_perm('users.delete_profile'):
            return Response({'msm':'Sem permissão de exclusão. Você será redirecionad(a) para pagina principal.' , 'status': 'danger' , 'return': True},status=HTTP_403_FORBIDDEN) 
    
        try:
            _collaborator = Collaborator.objects.get(id=pk, company_id=request.data["company_id"] , user_id=request.data["user_id"])
            _collaborator.is_active = request.data['is_active']
            _collaborator.save()
            
            return Response({'success': 'Heey! Ação efetuado acom sucesso.', 'status': True},status=HTTP_200_OK)
        except:
           return Response({'msm':"Oops! Houve um erro na exclusão.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND) 
...
class IndividualViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            queryset_individual = IndividualUsersSerializers(Individual.objects.get(user_id=user_id , company_id=companyId))
            queryset_contact = ContactsSerializers(Contact.objects.filter(company_id=companyId , user_id=user_id, main=1 , is_active=1).first() )
            queryset_bankaccount = BankAccountUsersSerializers(Bank_Account.objects.filter(company_id=companyId , user_id=user_id, main=1 , is_active=1).first())
            
            queryset = dict()
            queryset.update(queryset_individual.data)
            queryset.update({'bankaccount':queryset_bankaccount.data})
            queryset.update({'contact':queryset_contact.data})
    ...
    def create(self, request):
        ...
        try:
            serializerIndividual = IndividualUsersSerializers(data=request.data)
            ...
            return Response({'msm':"Oops! Houve um erro na inserção dos dados.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND)                
    ...
    def update(self, request, pk=None):
        ...
        try:
            _individual = IndividualUsersSerializers(Individual.objects.get(id=pk, user_id=user_id, company_id=companyId))
            serializerIndividual = IndividualUsersSerializers(_individual ,data=request.data)
            if serializerIndividual.is_valid():
                serializerIndividual.save()
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            _individual = IndividualUsersSerializers(Individual.objects.get(id=pk, user_id=user_id, company_id=companyId))
            _individual.is_active = request.data['is_active']
            _individual.save()
            return Response({'success': 'Heey! Ação efetuado acom sucesso.', 'status': True},status=HTTP_200_OK)
        except:
            return Response({'msm':"Oops! Houve um erro na exclusão dos dados.  Tente novamente..." , 'status': 'danger'},status=HTTP_404_NOT_FOUND)                
...
class BusinesViewSet(viewsets.ViewSet):
    ...
    def destroy(self, request, pk=None):
        ...
        try:
           queryset_busines = BusinesUsersSerializers(Busines.objects.get(id=pk, user_id=user_id , company_id=companyId))
        except:
            pass 
...
class PermissionViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            querysetPermission = PermissionSerializer(Permission.objects.all() , many=True)
            queryset = {
    ...
    def create(self, request):
        ...
        try:
            ...
            if not request.data:
                return Response({'error': 'Selecione as permissions necessarias.', 'status': False}, status=HTTP_404_NOT_FOUND) 
            ...
            return Response({'success': 'Alteração efetuada com sucesso.', 'status': True},status=HTTP_200_OK)
        except:
            return Response({'error': 'Error ao alterar as permissões, certifique-se se tudo ocorreu como o correto e tente novamente.', 'status': False},status=HTTP_404_NOT_FOUND) 
...
class UsersViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            queryset = UserBindSerializers(Bind.objects.all() , many=True , context={"request": request})
            return Response(queryset.data,status=HTTP_200_OK)
        except:
            return Response({'msm':'0 usuarios cadastrados.' , 'status': 'danger'},status=HTTP_404_NOT_FOUND) 
